...
namespace TutoringMarket.WebIdentity.Models.ViewModels
{
    public class TutorRequestModel
    {
        ...
        public async Task Init(IUnitOfWork uow, int id, UserManager<ApplicationUser> um, string name)
        {
            ...
            this.Comments = String.Format("Meine Daten sind: {0}E-Mail: <Gib hier deine E-Mail Addresse ein> {0}Telefon: <Telefonnummer> {0}Fächer, in denen ich Nachhilfe brauche: <Fächer> {0}{0} <Hier kannst du eventuelle Anmerkungen angeben>", Environment.NewLine);
